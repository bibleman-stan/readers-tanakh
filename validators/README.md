...
| `[DEVIATION]` | colometry/ | Editorial policy violation — break diverges from canon rule H1–H17 | Review; document exception or merge/split |
...
9. AST-parse verify before committing:
   ```bash
   PYTHONIOENCODING=utf-8 py -3 -c "import ast; ast.parse(open('validators/<layer>/<filename>.py').read())"
   ```
