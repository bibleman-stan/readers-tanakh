...
**Hebrew edit (`v2/he/`) → propagator (`scripts/propagate_editorial_layers.py`) for translit + interlinear → KJV regen (`scripts/regenerate_english.py`) for `v2/eng-kjv/` → HTML rebuild (`scripts/build_books.py`) → commit.**

`scripts/refresh_book.py` orchestrates all four stages. The pre-commit hook auto-runs `refresh_book.py --book <book> --build` whenever a v2/he file is staged.
