...
A colometric reading edition of the Hebrew Bible (Tanakh). The Masoretic Text is reformatted into **sense-lines (cola)** — each line one atomic thought, one image, one breath unit, motivated by Hebrew prosodic and grammatical structure. The edition is designed for oral delivery, devotional reading, and as an analytical substrate for compositional research.
...
**No free, web-native, fully-colometric Hebrew Tanakh exists.** No critical reading edition uses the te'amim as the structural prior with explicit override discipline. This project provides that edition and that methodology.
...
The disjunctive accents are the structural prior for line breaks. A baseline `v1-teamim` text is generated by parsing the disjunctive hierarchy directly (with two parsers: prose and *Sifrei Emet*). The editorial `v4-editorial` text departs from this baseline only with documented warrant.

This commitment is the project's principal differentiator. It is methodologically defensible (the accents are manuscript data, not modern conjecture), it provides a clear baseline for measuring editorial intervention rates, and it sidesteps the year-of-work cost of deriving Hebrew break-point criteria from scratch.

### 2. Four override criteria, applied with burden of proof

The four criteria — **atomic thought**, **single image**, **breath unit**, **Hebrew syntax** — are not free-floating intuitions. Each override of an accent-induced break must cite which criterion is invoked and why the accent system is wrong here.

A v4-editorial line break that matches no disjunctive accent and has no documented override warrant is a defect, to be either justified or reverted.
