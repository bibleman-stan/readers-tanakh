...
**Bidirectional atomic-thought test.** A line satisfies atomic-thought only if it is *referentially self-contained*, not just *grammatically forward-closed*. Lines whose content is anaphoric to prior context — deictic demonstratives (*הָאֵלֶּה*, *הַזֶּה*, *הַהוּא*) pointing to undefined antecedent, unresolved 3-person pronouns / pronominal suffixes, discourse-anaphoric particles opening the line (*לָכֵן*, *עַל־כֵּן*, causal *כִּי*, *אָז*) — fail the test even with a finite verb present, because the reader's understanding dangles BACKWARD into unresolved context. The Gen 22:1 case is canonical: *וַיְהִי אַחַר הַדְּבָרִים הָאֵלֶּה* ("and it came to pass after these things") merges with its apodosis because the frame is referentially anaphoric. Cataphoric reference (presentative *הִנֵּה* + indefinite NP introducing new content; *כֹּה אָמַר X* announcing speech to follow) does NOT fail the test. This is a §1.1 refinement of the generative principle, not a new merge-override — per [`../../../atu-method/docs/framework.md`](../../../atu-method/docs/framework.md) §1.1 and [`feedback_atu_test_is_bidirectional`](../../../atu-method/memories/feedback_atu_test_is_bidirectional.md). **Status: informational diagnostic, NOT precedence override.** The test surfaces candidates for editorial review; it does not adjudicate between competing rules or auto-fire validators (per gnt-reader precedent 2026-05-13; see [`feedback_bidirectional_is_diagnostic_not_override`](../../../../.claude/projects/c--Users-bibleman-repos-readers-tanakh/memory/feedback_bidirectional_is_diagnostic_not_override.md)).

**Rhetorical figures as constraints, not determinants.** Rhetorical figures (hendiadys, merism, parallelism, chiasm, anaphora, climax, etc.) have DEFAULT ATU dispositions that flow from their referential structure: hendiadys→merge (synonymy-coextension); merism→merge (polar-totality-coextension); parallelism / anaphora / climax→split each member (J1); chiasm→no independent force (structural-not-propositional). But each figure CONSTRAINS the candidate space; the atomic-thought test (bidirectional, above) DETERMINES the answer. Same asymmetry as grammar (§1.2): figures are another constraint layer over the determination engine, not parallel determination authorities. Per-instance context-discrimination required: the same lemma-pair can be merism→merge in one context (*שָׁמַיִם וָאָרֶץ* in totality formula) and J1→split in another (*שָׁמַיִם וָאָרֶץ* in distinct-domain narrative). The Tanakh `BONDED_LEMMA_PAIRS` closed-list (88 pair-instances; auto-extracted from `data/syntax-reference/hendiadys-lexicon.tsv`) is currently reference-only / dormant in validator code; activating it as a mechanical merge-trigger without per-instance context-discrimination would replicate the BoFM precedent failure (2026-05-13). Per [`feedback_rhetoric_figures_constrain_atu`](../../../atu-method/memories/feedback_rhetoric_figures_constrain_atu.md).
...
**Closed-list dormancy — `BONDED_LEMMA_PAIRS` (codified 2026-05-16).** The 88-pair Tanakh lexicon at [`data/syntax-reference/hendiadys-lexicon.tsv`](../../data/syntax-reference/hendiadys-lexicon.tsv) (auto-extracted to [`validators/_shared/hendiadys_lemma_pairs.py`](../../validators/_shared/hendiadys_lemma_pairs.py)) is **kept dormant** — imported as reference but NOT mechanically consulted by any validator. The active M1 mechanism is **per-verse Cat-B editorial judgment** surfaced via [`validators/colometry/validate_parallel_clause_split.py`](../../validators/colometry/validate_parallel_clause_split.py) and adjacent validators, applied case-by-case under M1's test (paraphrasability to unified hendiadys/merism + shared rhetorical weight). **Rationale:** (a) BoFM precedent (2026-05) where a context-blind lemma-pair closed-list fired hendiadys merges in non-hendiadic contexts produced a class of unrecoverable corpus errors and the lemma-list was retired from mechanical use; (b) the 88-pair surface is preserved as reference for potential future activation IF a context-discrimination protocol is built (e.g., Macula constituent-membership check + governing-verb shared-vs-distinct test + figure-class disambiguation); (c) dormancy means the 88 candidate pair-instances surface no validator output and some real editorial work goes undone — this tradeoff is accepted in favor of preventing context-blind false-positive merges. **Operational implication:** the alignment-check verdict EDITORIAL_ACK for closed-list `BONDED_LEMMA_PAIRS` is the correct classification; reactivation requires a separate canon revision documenting the context-discrimination protocol.
...
**Reference-grammar substrate.** Each rule's YAML footer carries a `references:` line citing two reference grammars where they ground the rule:

- **WO §N.M.K** — Waltke-O'Connor 1990, *An Introduction to Biblical Hebrew Syntax* (Eisenbrauns). 40-chapter advanced reference grammar; primary substrate. Per-chapter files at [`../05-resources/waltke-1990/`](../05-resources/waltke-1990/).
- **AC §N.M.K** — Arnold + Choi 2003, *A Guide to Biblical Hebrew Syntax* (Cambridge). 5-chapter intermediate reference grammar; consonant with W&O (cited in AC preface as primary influence) but with finer per-particle / per-clause-species granularity. Adds beyond W&O specifically at: Ch 4 (per-preposition + per-adverb treatments; dedicated §4.5 הִנֵּה / וְהִנֵּה section), Ch 5 (14-species subordinate-clause enumeration), §3.5.4 (Interruptions in Verbal Sequences — novel taxonomy for wayyiqtol chain breaks), §2.4 (Apposition 5-species frame complementary to W&O's 2-species). Per-chapter files at [`../05-resources/arnold-choi-2003/`](../05-resources/arnold-choi-2003/).

Both resource directories are gitignored (copyright); per-chapter files stay local. See each directory's README for navigation. Citation convention per Stan-directive 2026-05-13: any rule revision or new rule creation that draws on either grammar MUST cite at the YAML `references:` line plus a brief note in WHY/HOW WE KNOW prose. Citing the reference-grammar substrate strengthens defensibility and aligns with scholarly practice.
...
**Conformance to the universal rule template.** Tanakh §5 entries conform to [`../../../atu-method/docs/rule-template.md`](../../../atu-method/docs/rule-template.md) via the per-corpus-vocabulary clause therein: the universal-shape fields (Status / Category / Decidability / Layer / Rule / UD signature / Closed lists / Scope / Exclusions / Precedence / Examples / Implementation) are realized through the Tanakh-variant pattern of **prose-narrative body** (the five template fields above) **plus YAML-footer** (`primitive` / `detectors` / `closed_lists` / `fixture` / `cross_corpus` / `references`). Per-corpus vocabulary maps: Hebrew morpho-syntactic terms (wayyiqtol, casus pendens, Sifrei Emet routing, te'amim demotion) instantiate the universal-template's primitive/scope/exclusion slots. The structural alignment check at [`validators/canon/check_canon_alignment.py`](../../validators/canon/check_canon_alignment.py) validates the YAML-footer shape (detectors / closed_lists field presence + named-validator file presence + named closed-list source presence); the prose-narrative body answers the universal-template's substantive questions in narrative form. No migration of existing entries is required; the Tanakh-variant pattern is the recognized conforming shape.
...
references: WO §4.7 (Nominative Absolute and Vocative — vocative as syntactically distinct from clause core); AC §2.1.3 (Vocative as nominative subtype), §2.6.2 (Determination → Vocative)
```
...
references: WO §38.8 (Constituent Noun Clauses — clause-level continuation), §39.2.2 (Conjunctive-sequential Waw); AC §5.2 (Subordinate Clauses — 14 species enumerated: Substantival/Conditional/Final/Temporal/Causal/Comparative/Exceptive/Restrictive/Intensive/Adversative/Circumstantial/Concessive/Relative/Disjunctive)
```
...
references: WO §4.7 (Nominative Absolute = casus pendens — same construction), §16 (Personal Pronouns — resumptive-pronoun feature is the H15 trigger); AC §2.1.4 (Nominative Absolute — direct treatment of casus pendens construction)
```
