...
Run `5-machinery/scripts/ingest_tahot.py` to convert the vendored STEPBible TAHOT TSV (in `research/stepbible-tahot/`) into per-chapter `v0/prose/` files.
...
- `5-machinery/scripts/parse_teamim.py` — handles both the prose accent system (21 prose books + Job's prose frame 1:1–2:13 and 42:7–17) and the *Sifrei Emet* accent system (Psalms, Proverbs, and Job 3:1–42:6); per-book registry routes chapters to the appropriate breaker set.
...
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/propagate_editorial_layers.py --book 32-jonah
```
...
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/regenerate_english.py --book 32-jonah
```
...
Run `5-machinery/scripts/build_books.py` to regenerate `books/{book}.html` from `v2/` (cascade falls through to v1 for any chapter without a v2 file):

```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/build_books.py --book jonah
```
...
PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --baseline-check  # gate (what pre-commit runs)
```
...
2. Build a scanner in `5-machinery/scripts/` that finds it and cites evidence
