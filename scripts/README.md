# scripts/ — Tanakh Reader Tooling

This directory carries the pipeline scripts, diagnostic scans, validator-meta tools, fixture tests, and dormant-lexicon reference builders for the Tanakh reader. Scripts no longer in regular use live in [`scripts/archive/`](archive/) for provenance.

## Pipeline stages

The canonical editorial pipeline runs Stages 1–6. Per-stage detail in [`handoffs/04-editorial-workflow.md`](../handoffs/04-editorial-workflow.md).
...
| 6 — Validate | `run_validators.py` | Validator-suite runner (preferred entry point: `validators/run_all.py`) |
...
| Apply (spec-driven) | `apply_specs.py` | YAML-spec-driven apply via `validators/_shared/spec_runner.py` |
...
Standalone test fixtures (kept in `scripts/` rather than `tests/` per current convention).
