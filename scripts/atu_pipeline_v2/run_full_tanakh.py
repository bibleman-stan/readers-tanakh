...
def extract_clauses_for_chapter(api, book_name: str, chapter_num: int) -> list[dict]:
...
def render_v2_heb_format(groups: list[dict], book_stem: str, chapter_num: int) -> str:
    """Render ATU groups as v2/heb-format text (verse-headed, one ATU per line)."""
    # Group ATUs by verse_first (groups can span verses only if no rule
    # prevents it, but global same-verse guard does prevent it, so each
    # group lives in exactly one verse).
    by_verse: dict[int, list[str]] = {}
    for g in groups:
        by_verse.setdefault(g["verse_first"], []).append(g["text"])

    lines: list[str] = []
    for v in sorted(by_verse.keys()):
        lines.append(f"{chapter_num}:{v}")
        for atu in by_verse[v]:
            lines.append(atu)
...
def main():
    ...
    for book_folder in book_folders:
        ...
        for chapter_num, source_path in chapters:
            clauses = extract_clauses_for_chapter(api, book_name, chapter_num)
            if not clauses:
                skipped_chapters.append((book_folder, chapter_num))
                print(".", end="", flush=True)
                continue
            groups = apply_bindings(clauses)
            book_stem = source_path.stem
            rendered = render_v2_heb_format(groups, book_stem, chapter_num)
