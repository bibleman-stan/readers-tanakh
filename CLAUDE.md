...
| **Apply causes regression (4-layer integrity drops post-apply, validator FP rate spikes, etc.)** | **STOP. Do NOT build downstream-recovery tools first.** Revert the apply → root-cause WHY it caused regression (broken propagator? offset miscalculation? line-final whitespace? alignment-incompatible split point?) → fix the apply → re-attempt. Building a revert tool to "make it easier to roll back next time" is treating the symptom; the apply bug will keep producing rollbacks. Forward-only fix: address the upstream defect, then re-apply with the integrity gate verified post-apply BEFORE celebrating success. Cluster-agent "pass" reports don't substitute for the integrity gate — refresh_book exit codes don't trip on integrity drift; the dispatcher must run `verify_4_layer_sync.py --all` itself. | 2026-05-12 T2-apply attempt: 62 splits applied, integrity dropped 929/929 → 914/929, agent dispatcher reported all 5 clusters "pass" because refresh_book exit-code wasn't checking integrity. Reverted; built `scripts/apply_revert_lines.py` as recovery tool; Stan escalated "quit wasting time on the wrong approach". Root-cause investigation of apply→drift was the missed step |
