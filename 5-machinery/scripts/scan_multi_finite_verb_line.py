...
the merge. Per the 1-method/canon's generative principle (framework.md §1.1, 1-method/canon §1):
...
as 5-machinery/validators/colometry/validate_parallel_clause_split.py — non-parallel
sibling class).
...
Usage:
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/scan_multi_finite_verb_line.py
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/scan_multi_finite_verb_line.py --book 01-genesis
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/scan_multi_finite_verb_line.py --severity STRONG-SPLIT-CANDIDATE
"""
...
def _suppressor_cascade(line_tokens, clauses, anchor_count: int = None,
    ...
    # per 5-machinery/validators/_shared/hendiadys_lemma_pairs.py (M1-class merge).
    if head_a is not None and head_b is not None:
    ...
    # 5-machinery/scripts/test_h3_distinct_subject_signature.py): the dominant FP classes
