...
Runs adopted 5-machinery/validators, filters to STRONG findings per the adopted tag-set,
...
Phase 2 additions:
  validate_construct_chain         — STRONG-MERGE-CANDIDATE / divine_name subcase only
                                     ...
                                     ≥80% adoption gate cleared per 1-method/canon §7)
...
Sweep-scale audit warning (1-method/canon §7): if ≥5 changes are applied in a
single run, a WARNING is printed.

Usage:
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_validators.py --book 32-jonah
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_validators.py --all-books
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_validators.py --book 32-jonah --dry-run
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_validators.py --book 32-jonah --report-only
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_validators.py --book 32-jonah --force
"""
...
ADOPTED_VALIDATORS: dict[str, AdoptionSpec] = {
    ...
    # REVIEW-REQUIRED queue. Pre-tightening 1089 findings → post-tightening
    # ~984 findings (correct error direction: tightening suppresses FPs).
    "validate_causal_ki": {"STRONG-SPLIT-CANDIDATE"},
...
ALL_VALIDATORS: list[tuple[str, str]] = [
    ("5-machinery/validators/syntax/validate_maqqef_integrity.py",     "validate_maqqef_integrity"),
    ("5-machinery/validators/syntax/validate_line_final_tokens.py",    "validate_line_final_tokens"),
    ("5-machinery/validators/syntax/validate_compound_preposition_object.py", "validate_compound_preposition_object"),
    ("5-machinery/validators/syntax/validate_verb_object_bond.py",     "validate_verb_object_bond"),
    ("5-machinery/validators/colometry/validate_speech_intro_framing.py", "validate_speech_intro_framing"),
    ("5-machinery/validators/colometry/validate_wayehi_protasis.py",   "validate_wayehi_protasis"),
    # Partially adopted: divine_name subcase only; article_rectum + common_construct_ending
    # remain REVIEW-REQUIRED.
    ("5-machinery/validators/colometry/validate_construct_chain.py",   "validate_construct_chain"),
    # M4/subject-pronoun-orphan STRONG-MERGE arm only — other M4 patterns
    # remain REVIEW-REQUIRED for editorial judgment.
    ("5-machinery/validators/colometry/validate_short_orphan_line.py", "validate_short_orphan_line"),
    # ── Adopted 2026-05-04 §0.2.1 cleanup sweep ──
    ("5-machinery/validators/colometry/validate_bare_construct_head.py",   "validate_bare_construct_head"),
    ("5-machinery/validators/colometry/validate_bare_discourse_particle.py", "validate_bare_discourse_particle"),
    ("5-machinery/validators/colometry/validate_blessed_cursed_chain.py",  "validate_blessed_cursed_chain"),
    ("5-machinery/validators/colometry/validate_bonded_pair.py",           "validate_bonded_pair"),
    ("5-machinery/validators/colometry/validate_coordinated_object.py",    "validate_coordinated_object"),
    ("5-machinery/validators/colometry/validate_cross_verse_continuity.py", "validate_cross_verse_continuity"),
    ("5-machinery/validators/colometry/validate_genealogy_uniformity.py",  "validate_genealogy_uniformity"),
    ("5-machinery/validators/colometry/validate_list_formula_uniformity.py", "validate_list_formula_uniformity"),
    ("5-machinery/validators/colometry/validate_interrogative_clause.py",  "validate_interrogative_clause"),
    ("5-machinery/validators/colometry/validate_oath_formula.py",          "validate_oath_formula"),
    # ── Pre-existing ADOPTED-but-missing entry, repaired 2026-05-04 ──────
    ("5-machinery/validators/colometry/validate_complement_integrity.py",  "validate_complement_integrity"),
    # ── Registered 2026-05-04 post-Macula-pivot promotion sweep ──────────
    ("5-machinery/validators/colometry/validate_clause_nucleus_split.py",  "validate_clause_nucleus_split"),
    ("5-machinery/validators/colometry/validate_participial_speech_frame.py", "validate_participial_speech_frame"),
    ...
    ("5-machinery/validators/colometry/validate_causal_ki.py",             "validate_causal_ki"),
]
...
def _assert_adopted_in_all_validators() -> None:
    ...
    if missing:
        raise SystemExit(
            f"ERROR (1-method/canon §0.2.1 registry coupling): the following validator(s) "
...
def aggregate_findings(
    ...
    for validator_name, doc in validator_outputs:
        ...
        for finding in findings:
            ...
            tag = finding.get("tag", "")
            subcase = finding.get("subcase")  # may be None for 5-machinery/validators without subcases
...
def apply_mutations_to_lines(
    ...
    Conflict rule (1-method/canon §1 Decision Procedure step 4):
...
def _format_conflict_block(conflict: dict, conflict_num: int) -> str:
    ...
    return (
        ...
        f"- Resolution: **{resolution}** (1-method/canon §1 Decision Procedure step 4)\n"
    )
...
def build_chapter_report(
    ...
    if conflicts:
        lines_out.append("## Merge-vs-split conflicts (1-method/canon §1 step 4)\n")
        lines_out.append(
            "The following lines have a split candidate conflicting with a prior merge. "
            "Merge wins per 1-method/canon §1 Decision Procedure step 4. Review in v2/heb.\n"
        )
...
def process_book(
    ...
    """Run 5-machinery/validators and process all chapters in a book.

    In diff-apply mode (`diff_apply=True`), 5-machinery/validators are run against v2/heb
    ...
    only the new STRONG mutations the 5-machinery/validators emit on the current state.
    ...
    if total_applied >= 5:
        print(
            f"\nWARNING (1-method/canon §7 sweep-scale audit): {total_applied} changes applied "
            f"in this run. Per 1-method/canon §7, a sweep of ≥5 instances triggers a mandatory "
            f"audit. Run 5-machinery/validators/run_all.py --baseline-check and review the diff "
            f"before committing."
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Run 5-machinery/validators against v2/heb (current state, post any prior cascades) "
    ...
    if grand_total_applied >= 5:
        print(
            f"WARNING (1-method/canon §7): {grand_total_applied} total changes applied. "
            f"Mandatory sweep-scale audit required before commit. "
            f"Run: PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --baseline-check"
        )
