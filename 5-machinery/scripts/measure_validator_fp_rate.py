...
Two modes:
  ...
  --measure : load a labeled fixture TSV, invoke each referenced validator
              ...
              5-machinery/validators/.fp-baseline.json.
...
Fixture file (5-machinery/tests/fp-baseline-fixtures.tsv) columns:
...
Output (5-machinery/validators/.fp-baseline.json):
  {
    "fixture_count": 500,
    "labeled_count": int,
...
Usage:
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/measure_validator_fp_rate.py --sample
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/measure_validator_fp_rate.py --measure
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/measure_validator_fp_rate.py --measure --validator validate_short_orphan_line
"""
...
FIXTURE_FILE = REPO_ROOT / "5-machinery/tests" / "fp-baseline-fixtures.tsv"
OUTPUT_FILE = REPO_ROOT / "5-machinery/validators" / ".fp-baseline.json"
...
def fixture_template_rows(samples_by_cluster: dict[str, list[tuple[str, int, str]]]) -> list[str]:
    ...
    print(
        ...
        f"({len(validator_names)} 5-machinery/validators × {len(sampled_books)} books) "
...
def write_fixture_template(out_path: Path) -> int:
    ...
    print(
        ...
        f"({n_validators} 5-machinery/validators evaluated) to {out_path.relative_to(REPO_ROOT)}.",
        file=sys.stderr,
    )
    return n_verses
...
def measure(fixture_rows: list[dict], validator_filter: str | None = None) -> dict:
    """Run 5-machinery/validators per book + compare against ground-truth labels.

    Returns the output document destined for 5-machinery/validators/.fp-baseline.json.
    """
    if validator_filter:
    ...
    return {
        ...
        "5-machinery/validators": out_validators,
...
def main() -> int:
    ...
    p.add_argument("--fixture", type=Path, default=FIXTURE_FILE, help="path to fixture TSV (default: 5-machinery/tests/fp-baseline-fixtures.tsv)")
    p.add_argument("--output", type=Path, default=OUTPUT_FILE, help="path to output JSON (default: 5-machinery/validators/.fp-baseline.json)")
    ...
    if args.measure:
        ...
        n_validators = len(doc["5-machinery/validators"])
        labeled = doc["labeled_count"]
        gt = doc["ground_truth_summary"]
        print(
            f"Measured {labeled} labeled fixture rows across {n_validators} 5-machinery/validators; "
