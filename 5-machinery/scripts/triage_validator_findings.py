...
Workflow:
  ...
  6. (Future) An aggregator parses the marked ballot back into per-rule_id TP rate
     and recommends STRONG promotion when ≥80% TP per 1-method/canon §7.

Usage:
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/triage_validator_findings.py \\
...
# 6 cluster groups (from CLAUDE.md / 4-process/02-operational-protocols.md).
...
def discover_validator(name: str) -> Path | None:
    """Find validate_<name>.py under 5-machinery/validators/syntax or 5-machinery/validators/colometry."""
    candidates = [
...
def normalize_finding(f: dict) -> dict:
    ...
    Standard shape (most 5-machinery/validators): file, line, next_line, severity, rule_id,
...
def main() -> int:
    ...
    if vp is None:
        return _err(f"validator not found: validate_{args.validator}.py under 5-machinery/validators/{{syntax,colometry}}/")
