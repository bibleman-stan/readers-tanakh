...
STOP-AND-SURFACE on 1-method/canon §1 line 151 conflict with passage-level discipline).
...
Usage:
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/audit_rendered_output.py \\
...
1. **Forward grammatical closure** — grammatically complete in Hebrew terms:
   ...
   - **Verbless / nominal-predicate clauses**: subject + non-verbal predicate (NP/PP/Adj) in juxtaposition counts as closed (Hebrew has no overt copula in present-tense verbless clauses). NOTE: this applies ONLY when the nominal head is predicated (subject + predicate). A bare construct-state noun (e.g., דְּבַר alone, awaiting genitive YHWH on the next line) is NOT a nominal predication — it FAILS forward closure (M3 bare-governor indivisibility per 1-method/canon §5).
...
- **Discourse particles** (לָכֵן / וְעַתָּה / אָז / הִנֵּה when followed by content on same line): they lead content; particle + its governed content = ONE ATU. Never SPLIT a particle from immediately following content. If a particle appears alone on a line (bare), verdict = MERGE-WITH-NEXT per forward-closure failure (1-method/canon §5 H14 + M3).
