...
# -*- coding: utf-8 -*-
"""
Validate 1-method/canon Rule M1 — Bonded Pair.

M1 (1-method/canon §3; Layer 3 editorial rule):
...
Severity:
  STRONG-MERGE-CANDIDATE (M1 is closed-list per 1-method/canon §1).
...
Usage:
    PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/colometry/validate_bonded_pair.py
    PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/colometry/validate_bonded_pair.py --book genesis
    PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/colometry/validate_bonded_pair.py --v2
    PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/colometry/validate_bonded_pair.py --json
"""
...
# ---------------------------------------------------------------------------
# Canonical bonded pairs from 1-method/canon §1 M1 + variants. Each tuple represents
# an ordered pair (skeleton1, skeleton2). The pair is detected when:
#   - Line N ends with skeleton1
#   - Line N+1 starts with וְ + skeleton2
#
# Building the lexicon from 1-method/canon §1 examples:
...
def scan_file(path: Path, verbose: bool = False) -> list[dict]:
    ...
    for i, line in enumerate(lines):
        ...
        annotation = (
            ...
            f"hendiadys / merism / bonded rhetorical image (1-method/canon §3 M1)."
        )
...
def main():
    ...
    print(f"Reference: 1-method/canon §3 M1 (hendiadys / merism / bonded pair)")
    print("=" * 72)
