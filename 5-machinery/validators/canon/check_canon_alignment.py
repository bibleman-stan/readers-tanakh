...
verifies that what 1-method/canon §5 entries NAME actually EXISTS in the validator code.
Mechanical / structural only — does NOT verify semantic alignment (whether
predicate logic implements what 1-method/canon prose claims).
...
Verdict taxonomy:
  ...
  NO_IMPL         — 1-method/canon names a validator file that doesn't exist
  DRIFT           — validator file exists; named closed-lists or fields missing
  PARTIAL         — some named items present, others missing
  EDITORIAL_ACK   — 1-method/canon explicitly declares no auto-applier / dormant lexicon

Structural precedent: 5-machinery/validators/colometry/validate_canon_retirement_residue.py
(scans 1-method/canon prose, applies pattern matching, reports drift candidates).
"""
...
#   list-item form:    "  - 5-machinery/validators/foo/bar.py"
#   inline-prose form: "detectors: sub-check inside 5-machinery/validators/foo/bar.py"
...
INLINE_PATH_RE = re.compile(r"(5-machinery/validators/[\w/]+\.py)")
...
# Cross-validator constants frequently live in 5-machinery/validators/_shared/*.py rather
...
# imports it.
SHARED_DIR = Path(__file__).resolve().parents[2] / "5-machinery/validators" / "_shared"


def parse_canon_entries(text: str) -> list[dict]:
    """Split the 1-method/canon by ### Rule HN — ... headers. Return a list of dicts
...
def extract_yaml_fields(body_lines: list[str]) -> dict:
    ...
    Detector paths are matched in two forms:
      (a) list-item form — '  - 5-machinery/validators/foo/bar.py'
      (b) inline-prose form — 'detectors: sub-check inside 5-machinery/validators/foo/bar.py'
...
def check_closed_list_in_source(detector_path: str, list_name: str) -> bool:
    """Grep the validator source AND 5-machinery/validators/_shared/*.py for the closed-list
    ...
    in 5-machinery/validators/_shared/ rather than in any single detector file.

    Match is prefix-tolerant: `\\bNAME` (word-start boundary, no trailing
    boundary) so that a 1-method/canon name like SIFREI_EMET_BOOKS matches a source
...
def classify(entry: dict) -> tuple[str, list[str]]:
    ...
    if missing_lists:
        for cl in missing_lists:
            evidence.append(f"closed-list '{cl}' named in 1-method/canon, not found in detector source")
