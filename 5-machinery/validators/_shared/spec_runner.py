...
Specs are YAML files in `5-machinery/validators/specs/` declaring trigger conditions,
...
Usage:
    ...
    runner = SpecRunner('5-machinery/validators/specs/')
    findings = runner.run_corpus('data/text-files/v2/heb/')

CLI: see 5-machinery/scripts/run_validators.py.
...
imports `atu_method.*` only from build 5-machinery/scripts (build_books.py,
...
def _g_next_wayehi_ken(l_n, l_n1, ctx):
    ...
    וַיְהִי־כֵן. The formula is its own atomic thought (1-method/canon §1, S3 pattern 1)
...
def _g_next_verbless_predication(l_n, l_n1, ctx):
    ...
    clause (subject + nominal predicate). Per 1-method/canon §5 H5b: verbless
...
def _g_next_interrogative_clause(l_n, l_n1, ctx):
    ...
    pronoun/particle. Per 1-method/canon §5 H5b, interrogative-headed clauses are
...
def _g_next_imperative_clause(l_n, l_n1, ctx):
    ...
    cohortative verb. Per 1-method/canon §5 H5b, imperative-headed clauses are
