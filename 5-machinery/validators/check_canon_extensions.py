...
Detects 1-method/canon extensions (1-method/canon §7-style triggers) in staged colometry-canon
...
This script:
  1. Reads staged 1-method/canon diffs (private/01-method/colometry-canon.md).
...
# Both the Layer-3 methodology 1-method/canon AND the Layer-1 grammar surface table need
# the same audit gate: closed-list extensions to either are scope-changing
# additions per 1-method/canon §7. Imported from GNT-Reader sibling 2026-05-05.
CANON_FILES = [
...
def main() -> int:
    ...
    diff = get_canon_diff()
    if not diff:
        return 0  # no 1-method/canon changes staged
    ...
    print(
        "Per 1-method/canon §7 mandatory-audit triggers (and the BoFM-precedent "
        "smuggling case), this commit introduces 1-method/canon extensions that "
