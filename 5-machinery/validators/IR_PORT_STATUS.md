...
(`5-machinery/validators/_shared/macula_constituents.py`).
...
| `validate_circumstantial_clause` | L3 | RETIRED | 2026-05-07 | Removed per Stan REVIEW-REQUIRED ban. Audit cycle: 30% TP baseline → 43% with PP-coordination guard → 73% with closed-list trigger narrowed to canonical SJ5 markers (וְהִנֵּה / וְהוּא / וְהִיא / וְהֵם / וְהֵן + content). Cluster-stratified Sonnet audit (n=26) confirmed FPs are diverse (apodosis-head, logical-conclusion, syntactically-incomplete, core-bicolon-predicate, object-complement) — no single guard reaches 80%. Bicolon parallelism (~80% TP class) covered by `validate_parallel_clause_split` (Hpar). Substantive-adjunct judgment stays editor-side without a validator. |
...
## Spec layer (5-machinery/validators/specs/*.yaml)
...
- 13 5-machinery/validators audited & ruled NO-PORT or DEFER (M4 weqatal arm, maqqef_integrity, circumstantial_clause, causal_ki, complement_integrity, oath_formula, doc_pointers, bare_discourse_particle, canon_retirement_residue, bonded_pair, blessed_cursed_chain, parallel_series_uniformity, interrogative_clause)
- 4 5-machinery/validators left NOT-AUDITED (all 0-2 findings, low priority)
...
- **Pre-commit overhead**: each commit triggers `run_all.py --baseline-check` (~1 min for all 26 5-machinery/validators). Chained `update-baseline + commit` doubles to ~2 min. Multi-commit waves accumulate quickly; minimize commit count when possible.
